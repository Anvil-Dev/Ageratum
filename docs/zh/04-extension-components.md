...
**扩展组件**（Extension Component）是 Ageratum 文档中可渲染的自定义块级元素。通过注册 `MDExtensionComponentFactory`，开发者可以使用两种语法在文档中插入自定义渲染内容：
...
**智能宽度分配：**
- 优先使用子组件声明的期望宽度（通过 `getPreferredWidth()` 方法）
- 剩余空间由没有期望宽度的组件平均分配
- 这使得固定宽度组件（如图片、物品）可以与弹性组件（如文本）混合使用
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| `direction` | 字符串 | ✗ | `horizontal` | 排列方式：`horizontal`（水平并排）/ `vertical`（垂直堆叠） |
| `halign` | 字符串 | ✗ | `left` | 水平对齐：`left` / `center` / `right`。<br/>水平排列时：控制整行在 `maxX` 内的对齐；垂直排列时：控制每个子组件在 `maxX` 内的对齐 |
| `valign` | 字符串 | ✗ | `top` | 垂直对齐：`top` / `center` / `bottom`。<br/>仅在 `direction=horizontal` 时生效，用于控制子组件在该行高度内的上/中/下对齐 |

> **提示：**
> - 水平排列时，行组件会自动计算每个子组件的宽度。
> - 子组件可以通过重写 `getPreferredWidth()` 方法声明期望宽度。
> - 没有声明期望宽度的组件会平均分配剩余空间。
> - 子组件之间会有 4 像素的固定间距。
